...
namespace pio
{
	namespace Math
	{
		...
		void CalcCameraPose(const glm::vec3& viewDir, glm::vec3& cameraRight, glm::vec3& cameraUp, glm::vec3& cameraForward)
		{
			cameraRight = glm::normalize(glm::cross(viewDir, World::Up));
			cameraUp = glm::normalize(glm::cross(cameraRight, viewDir));
			cameraForward = -viewDir;
		}

		glm::mat4 CameraViewMat(const glm::vec3& position, const glm::vec3& viewDir, const glm::vec3& cameraUp)
		{
			return glm::lookAt(position, position + viewDir, cameraUp);
		}

		glm::vec3 BillboardRotation(const glm::mat4& camViewMat)
		{
			auto rotMat = glm::mat3(glm::inverse(camViewMat));
