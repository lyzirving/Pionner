...
namespace pio
{
	...
	bool GLRenderAPI::CopyFrameBufferDepth(const Ref<FrameBuffer>& src, const Ref<FrameBuffer>& dst)
	{
		if(!src)
		{
	...
	void GLRenderAPI::ReleaseResource(ResourceGCDelegate&& resource)
	{
		if (resource.Id == 0)
		{
			LOGW("warning! resource[%s][%s] has not been initialized", resource.Name.c_str(), RenderResourceTypeStr(resource.Type));
			return;
		}
		switch (resource.Type)
		{
			...
			{
				LOGD("delete buffer[%s][%u][%s]", resource.Name.c_str(), resource.Id, RenderResourceTypeStr(resource.Type));
				glDeleteBuffers(1, &resource.Id);
				break;
			}
			case RenderResourceType::VAO:
			{
				LOGD("delete vertex array[%s][%u][%s]", resource.Name.c_str(), resource.Id, RenderResourceTypeStr(resource.Type));
			...
			{
				LOGD("delete fame buffer[%s][%u][%s]", resource.Name.c_str(), resource.Id, RenderResourceTypeStr(resource.Type));
				glDeleteFramebuffers(1, &resource.Id);
				break;
			}
			case RenderResourceType::Texture:
			{
				LOGD("delete texture[%s][%u][%s]", resource.Name.c_str(), resource.Id, RenderResourceTypeStr(resource.Type));
				glDeleteTextures(1, &resource.Id);
				break;
			}
			case RenderResourceType::RenderBuffer:
			{
				LOGD("delete render buffer[%s][%u][%s]", resource.Name.c_str(), resource.Id, RenderResourceTypeStr(resource.Type));
				glDeleteBuffers(1, &resource.Id);
				break;
			}
			case RenderResourceType::Shader:
			{
				LOGD("delete shader[%s][%u][%s]", resource.Name.c_str(), resource.Id, RenderResourceTypeStr(resource.Type));
				glDeleteProgram(resource.Id);
				break;
			}
			default:
				LOGE("err! invalid resource [%s][%u][%s]", resource.Name.c_str(), resource.Id, RenderResourceTypeStr(resource.Type));
				break;
		}
	}
}